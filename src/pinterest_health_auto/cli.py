...
def discover_topics(limit: int = 10) -> None:
    """Fetch and persist trending US health topics."""
    db = _db()
...
def generate_pins(topic_id: int, topic_name: str, count: int = 8) -> None:
    """Generate pin drafts + images + schedule."""
    db = _db()
    pipeline = Pipeline(db)
...
def publish_due() -> None:
    """Publish due pins through API or CSV fallback."""
    db = _db()
    pipeline = Pipeline(db)
...
@app.command()
def run_daily(limit_topics: int = 5, pins_per_topic: int = 5, publish_now: Optional[bool] = False) -> None:
    db = _db()
    pipeline = Pipeline(db)
    topic_ids = pipeline.seed_topics(limit=limit_topics)
    topic_rows = db.list_topics(limit=limit_topics)
    for row in topic_rows:
        pipeline.generate_pins_for_topic(row["id"], row["name"], idea_count=pins_per_topic)
    if publish_now:
        pipeline.publish_due()
    console.print(f"Run completed. Topics seeded: {len(topic_ids)}")
